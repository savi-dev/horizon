...
class AddTenant(forms.SelfHandlingForm):
    ...
    def handle(self, request, data):
        try:
            api.add_tenant_user_role(request,
                                     ...
                                     data['user_id'],
                                     data['role_id'])
            messages.success(request, _('Successfully added project to domain.'))
        except:
            exceptions.handle(request, _('Unable to add project to domain.'))
        return shortcuts.redirect('horizon:syspanel:domains:projects',
                                  domain_id=data['domain_id'])


class CreateDomain(forms.SelfHandlingForm):
...
class UpdateDomain(forms.SelfHandlingForm):
